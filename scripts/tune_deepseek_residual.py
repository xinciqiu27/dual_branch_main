...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Hyperparameter search for DeepSeek residual-text experiments.")
    p.add_argument("--data-dir", type=str, default="data")
    p.add_argument("--base-save-dir", type=str, default="outputs/tune_ds_res")
...
def build_search_space() -> list[dict[str, object]]:
    # Prioritize the historically strong region first so DeepSeek residual
    # experiments remain comparable with earlier high-Recall runs.
    grid = {
        "lr": [5e-4],
        "dropout": [0.1],
        "emb-dim": [128],
        "n-layers": [2],
        "lambda-exp": [0.1, 0.2, 0.3],
        "lambda-imp": [0.0],
        "lambda-tss": [0.25],
        "negative-mode": ["strict_global", "hard"],
        "fusion-mode": ["mlp", "avg", "learnable"],
        "batch-size": [128, 256],
        "stage1-epochs": [10],
        "stage2-epochs": [10],
        "stage3-epochs": [20],
...
def config_to_name(idx: int, cfg: dict[str, object]) -> str:
    neg_tag = "sg" if cfg["negative-mode"] == "strict_global" else "hd"
    if cfg["fusion-mode"] == "avg":
        fuse_tag = "a"
    elif cfg["fusion-mode"] == "learnable":
        fuse_tag = "l"
    else:
        fuse_tag = "m"
    return (
...
def main() -> None:
    ...
    search_results.sort(
        key=lambda x: (
            ...
            x["best_val_metrics"].get("Recall@20", 0.0),
        ),
        reverse=True,
    )
    ...
    aggregate.sort(
        key=lambda x: (
            ...
            x["val_metrics_mean"].get("Recall@20", 0.0),
        ),
        reverse=True,
    )
